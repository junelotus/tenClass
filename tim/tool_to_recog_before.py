...
python_path = "/Users/mtest/opencv_bitbucket/timestampocr"# os.environ["PYTHONPATH"].split(':')[0]
...
def Loss3(model,img,class_tool=None,deblur_tool=None,frame_counter = 0,strat_list = [],index_1 = 0,pre_time=0,current_time=0):
...
def toRecordDealy(listsOfImg, cnt, thIfDelay, preImage, pre_time,pre_id, dealy_ms, counter_all,
    ...
    # print('loss_2 = ',loss2)
    loss3 = Loss3(preImage,currentImage,class_tool,deblur_tool,couter_frame_wode,pre_time=pre_time,current_time=current_time)#,strat_list=strat_list,index_1=index_)
    # print('loss_3 = ',loss3)
...
def binary_find_the_same_frame(first_reserve,current_frame,cnt,class_tool,deblur_tool,detectrou_tool,flag = 'headortail',listsOfImg_position=[],pre_time= -1,imageModel=None,must_tail=False,process_id=0):
    ...
    # if process_id == 2:# or process_id == 0:
    # print('begin_time,end_time,flag,must_tail',first_reserve[0][0],first_reserve[-1][0],flag,must_tail)
    ...
    while begin+1 < end:
        ...
        loss3 = Loss3(current_frame,now_image,class_tool,deblur_tool,pre_time =pre_time,current_time=first_reserve[middle][0])#,couter_frame_wode)#,strat_list=strat_list,index_1=index_)
...
def update_pre_and_first_reserve(first_reserve,pre_image,currentImage,pre_time,class_tool,deblur_tool,detectrou_tool,cnt,dealy_ms,listsOfImg_position=[],imageModel=None,first_200=False,process_id=0):#,last_200 =False):
    
    loss3 = Loss3(pre_image,currentImage,class_tool=class_tool,deblur_tool=deblur_tool,pre_time=pre_time,current_time=first_reserve[-1][0])#,couter_frame_wode)#,strat_list=strat_list,index_1=index_)
    ...
    if False == loss3:
        ...
        first_reserve,time_now = binary_find_the_same_frame(first_reserve, pre_image,cnt,class_tool,deblur_tool,detectrou_tool,flag='tail',listsOfImg_position=listsOfImg_position,pre_time=pre_time,imageModel=imageModel,must_tail=first_200,process_id=process_id)
        if first_200 == True:
        ...
        loss3 = Loss3(currentImage,now_image,class_tool,deblur_tool,pre_time= first_reserve[0][0],current_time=first_reserve[-1][0])#,couter_frame_wode)#,strat_list=strat_list,index_1=index_))#,couter_frame_wode)#,strat_list=strat_list,index_1=index_)
        ...
        first_reserve,time_now = binary_find_the_same_frame(first_reserve,currentImage,cnt,class_tool,deblur_tool,detectrou_tool,flag='head',listsOfImg_position=listsOfImg_position,imageModel=imageModel,process_id=process_id)#,must_tail=(first_200 or last_200))
...
def half_of_video_deal(begin_frame_counter,end_frame_counter,path,comm,process_id=0):
    list_point = []
    import datetime

    imageModel, contours_model_result = getModelJEPGAnd15Positions(python_path+'/model_8.jpg')
    ...
    while capture.isOpened():
        if len(first_reserve) == 0 or end_time - first_reserve[0][0] < 200:
            ...
            first_reserve,pre_image,pre_time,dealy_ms,first_200 = update_pre_and_first_reserve(first_reserve,pre_image,currentImage,pre_time,class_tool,deblur_tool,detectrou_tool,contours_model_result[0],dealy_ms,listsOfImg_position=listsOfImg_position,imageModel=imageModel,first_200=first_200,process_id=process_id)
            #comm.barrier()
    ...
    currentImage, thIfDelay = getDelayResultImage(listsOfImg, contours_model_result[0])
    first_reserve,pre_image,pre_time,dealy_ms,_ = update_pre_and_first_reserve(first_reserve,pre_image,currentImage,pre_time,class_tool,deblur_tool,detectrou_tool,contours_model_result[0],dealy_ms,listsOfImg_position=listsOfImg_position,imageModel=imageModel,process_id=process_id)#,last_200=True)
...
python_path = "/Users/mtest/opencv_bitbucket/timestampocr"#os.environ["PYTHONPATH"].split(':')[0]
    ...
    # path = '/Users/xhj/Downloads/LiveBroadcasting-720_1280-2000-15-TCOff_FREEZE_RATE.mp4'
...
def main_to_call(path,begin_frame,end_frame,comm,process_id=0):

    # cap = cv2.VideoCapture(path)
    # frame_counter = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
    # 
    # cap.release()
    # half_of_video_deal(begin_frame,end_frame,path)
    # if  process_id != 2:# or process_id==3  or process_id==1):
    #     return 
    print('process_id,begin_frame,end_frame',process_id,begin_frame,end_frame)
    # if process_id != 1:# or process_id != 1:#not(process_id == 5 or process_id == 5):
    #     return  
    return half_of_video_deal(begin_frame,end_frame,path,comm,process_id)
    # th1 = threading.Thread(target= half_of_video_deal, args=(begin_frame,end_frame,path))#int(begin_frame/+end_frame)/2,path))
    # th1.start()
    # th2 = threading.Thread(target= half_of_video_deal, args=(int(begin_frame/+end_frame)/2+1,int(end_frame),path))
    # th2.start()
    # th1.join()
    # th2.join()
    ...
    for i in range(len(first_reserve_list)):
        print(first_reserve_list[i][0:2])
    ...
    result_half.append(dealy_ms)
    print(np.sum(result_half, axis = 0))# 最终结果，数值，不是百分比
