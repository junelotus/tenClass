...
def constructCircleToJudgeCornerDot(img_to_correct_binary,xx,yy,radius,shape_harr,thread_angle = 6):
    pixel_list = []
    angle = 0.0
    begin_1 = False
    begin_0 = False
...
def choseFinalFornerDotList(cornerdots_list,x_or_y,coordinate_limit_1,coordinate_limit_2,thread_var,thread_to_travel=5):
    # cornerdots_list = np.array(cornerdots_list).reshape((-1,3))
    # cornerdots_list = [x[0] for x in cornerdots_list]
    # cornerdots_list = cornerdots_list.tolist()
    ...
    while end_x < len(cornerdots_list):
        while begin_index < end_x and thread_to_travel < abs(cornerdots_list[end_x][index_of_xory]-cornerdots_list[begin_index][index_of_xory]):
            begin_index = begin_index+1
        if begin_index >= end_x:
            end_x = end_x+1
            continue
        sub_list = cornerdots_list[begin_index:end_x+1]
        mean_1 = (np.array(sub_list).mean(axis=0))
        var_1 = (np.array(sub_list).var(axis=0))
...
def choseFinalFornerDot(list_1,list_2):
    ...
    #         break
    if list_2 is None:
        return (np.array(list_1).mean(axis=0))
...
def calGradient(img,list_of_position):
    ...
    #     grad_x = 
    return True
def correctImageByPerspectiveOrHough(img_to_correct,img_result,flag_of_path = 0):
    ...
    showImg('absY',absY)
    # dst =  img_to_correct.copy()#np.ones((shape_harr[0],shape_harr[1],1),dtype=np.uint8)
    counter = 0
    for i in range(shape_harr[0]):
        for j in range(shape_harr[1]):
            if absX[i][j] > 100 and absY[i][j] > 100:
                if i <= threshold_to_harr_x or i >= shape_harr[0] - threshold_to_harr_x :
                    corrdia_list.append([i,j])
                    # cv2.circle(dst,(j,i),8,(255,0,0))
                    counter = counter+1
                elif j <= threshold_to_harr_y or j >= shape_harr[1] - threshold_to_harr_y :
                    corrdia_list.append([i,j])
                    # cv2.circle(dst,(j,i),8,(0,255,0))
                    counter = counter+1
    # print('counterjune',counter)
                # dst[i][j] = 255
    # dst=cv2.addWeighted(absX,0.5,absY,0.5,0)
    ...
    for position in corrdia_list:
        ...
        if x <= threshold_to_harr_x or x >= shape_harr[0] - threshold_to_harr_x :
            begin,end ,pixel_list,white_counter,black_counter = constructCircleToJudgeCornerDot(img_to_correct_binary,x,y,14,shape_harr)
    ...
    # lu_list.append([1,2])
    # lb_list.append([100,30])
    # rb_list.append([75,26])
    # ru_list.append([94,94])
    if len(lu_list) > 0 :
    ...
    if len(canvas_point) == 4:
        x = min(min(canvas_point[0][0],canvas_point[1][0]),min(canvas_point[2][0],canvas_point[3][0]))
        y = min(min(canvas_point[0][1],canvas_point[1][1]),min(canvas_point[2][1],canvas_point[3][1]))
        
        canvas_point1 = canvas_point.copy()
        canvas_point1[0] =canvas_point1[0]-[x,y]
        canvas_point1[1] = canvas_point[3]-[x,y]
        canvas_point1[2] = canvas_point[1]-[x,y]
        canvas_point1[3] = canvas_point[2]-[x,y]
        canvas_point = canvas_point1.copy()
        #print('solution')
        ...
        # dst = np.array([[0,0],[height ,0],[height,width],[0,width]])
        dst = np.array([[0, 0],[0, 900],[500, 0],[500, 900]],np.float32)
        ...
        a_begin = int(min(canvas_point[0][0],canvas_point[3][0]))
        a_end  = int(max(canvas_point[1][0],canvas_point[2][0]))

        b_begin = int(min(canvas_point[0][1],canvas_point[1][1]))
        b_end  = int(max(canvas_point[2][1],canvas_point[3][1]))
        img_to_correct = img_to_correct[a_begin:a_end,b_begin:b_end]
        AffineImg = cv2.warpPerspective(img_to_correct, AffineMatrix1,(500,900))#(int( width),int( height)))# (img_to_correct.shape[1], img_to_correct.shape[0]))#

        result_perspective = AffineImg #drawPictureUseCornerDotOri(AffineImg,img_to_correct,AffineMatrix,width,height,canvas_point)
