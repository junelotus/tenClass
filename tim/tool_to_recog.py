...
# The SSIM function in the later version of 0.16.1 has been moved and renamed
from packaging import version
import skimage
if version.parse(skimage.__version__) >= version.parse('0.16.1'):
...
def getModelJPEGAnd15Positions(path):
...
def findROI(image_color, imageModel, w_, h_, l2_loss_1, l2_loss_2, ratio):
    ...
    for cnt in contours:
        ...
        # showImg("imgCurrent",imgCurrent)
        # showImg("imageModel",imageModel)
        L2Loss_result = L2Loss(imgCurrent, imageModel)
        
        
        # showImg("imgCurrent_"+str(i),imgCurrent,10000)
        if not ((L2Loss_result > l2_loss_1) and (L2Loss_result < l2_loss_2)) or \
                float(imgCurrent.shape[0]) / float(imgCurrent.shape[1]) < ratio:
            continue
        print("L2Loss_result",L2Loss_result)
        print("l2_loss_1",l2_loss_1)
        print("l2_loss_2",l2_loss_2)
        print("float(imgCurrent.shape[0]) / float(imgCurrent.shape[1]",float(imgCurrent.shape[0]) / float(imgCurrent.shape[1]))
...
def pregetTimeresultOrDelay(img_color, imageModel, flag_of_path=0, kind='timeresult',listsOfImg_position=[], detectrou_tool=None):
    ...
    if len(listsOfImg_position) == 0:
        ...
        detectrou_tool.set_path_or_image(img_color, python_path+'/model_8.jpg')
    ...
    for img_sub,position in zip(img_result,img_result_position):
        ...
        # print("sizesizesizesize",len(listsOfCntNormalOrder))
        ...
        if len(listsOfImg_sub) >= 1:
            # showImg('listsOfImg_sub',listsOfImg_sub[0])
...
def getDelayResultImage(listsOfImg, cnt, thIfDelay=1):
    ...
    while i >= 0:
        if listsOfImg[i][1] == 'perspective' or listsOfImg[i][1] == 'cnt':
            ...
            # print('counter_white',counter_white,img_curent_sub.shape[0]*img_curent_sub.shape[1])
            # print('counter_white',counter_white/(img_curent_sub.shape[0]*img_curent_sub.shape[1]))
            
            if counter_white / (img_curent_sub.shape[0] * img_curent_sub.shape[1]) > 0.4 and counter_white / (img_curent_sub.shape[0] * img_curent_sub.shape[1]) <= 0.9:
...
def recog(img,model,class_tool=None,deblur_tool=None,pre_time=0,current_time=0):
    # showImg(str(current_time),img)
    # showImg(str(pre_time),model)
    time_now = datetime.datetime.now() 
    img = img[0: img.shape[0],int(img.shape[1]/5*2+3):img.shape[1]-10]
    model = model[0: model.shape[0],int(model.shape[1]/5*2+3):model.shape[1]-10]
    #if pre_time == 0:
    # showImg(str(current_time),img)
    # showImg(str(pre_time),model)
    true_of_flag=0
    ...
    for i in range(3):
        for j in range(3):
            ...
            # img_gery_1_copy = img_gery_1.copy()
            # img_gery_2_copy = img_gery_2.copy()

            # if img_gery_1_copy.ndim == 3:
            #     img_gery_1_copy = cv2.cvtColor(img_gery_1_copy, cv2.COLOR_RGB2GRAY)
            #     # rct, img_gery_1_copy = cv2.threshold(img_gery_1_copy, 0, 1, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
            # if img_gery_2_copy.ndim == 3:
            #     img_gery_2_copy = cv2.cvtColor(img_gery_2_copy, cv2.COLOR_RGB2GRAY)
            #     # rct, img_gery_2_copy = cv2.threshold(img_gery_2_copy, 0, 1, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
            
            # if pre_time == 13907:
            # showImg(str(current_time),img_gery_1)
            # showImg(str(pre_time),img_gery_2)
    ...
    # print('huafei nei',datetime.datetime.now()-time_now)
    return true_of_flag#, false_of_flag
...
#     jiange_1 = int(shape[1]/3)
#     jiange_2 = int(shape[0]/3)
#     # if 46388 == pre_time and current_time == 46588:
#     #     showImg(str(current_time),img)#img_gery_1)
#     #     showImg(str(pre_time),model)#img_gery_2)
#     # false_of_flag = [0,0,0]
#     # true_of_flag = [0,0,0]
#     for i in range(3):
#         for j in range(3):
...
#             img_gery_2 = model[jiange_2*j:jiange_2*(j+1),jiange_1*i:jiange_1*(i+1)]
#             # if 46388 == pre_time and current_time == 46588:
#             #     showImg(str(current_time)+'sub',img_gery_1)#img_gery_1)
#             #     showImg(str(pre_time)+'sub',img_gery_2)#img_gery_2)
#             # print('img_gery_11',img_gery_1)
#             if (img_gery_1.ndim == img_gery_2.ndim ):
#                 simm = compare_ssim(img_gery_1,img_gery_2,multichannel=True)
#                 # print('simm',simm)
#                 if simm >= 0.90:
...
#                 img_gery_1 = np.array(img_gery_1[0] * 0.5 + 0.5) * 255
#                 # img_gery_1
#                 # img_gery_1 = img_gery_1.astype(np.int32)
#                 # print('img_gery_12',img_gery_1)
...
#                 img_gery_2 = deblur_tool.to_get_result()
#                 img_gery_2 = np.array(img_gery_2[0] * 0.5 + 0.5) * 255
#                 # img_gery_2 = img_gery_2.astype(np.int32)
#                 # if 120615 == pre_time:
#                 # showImg('img_gery_2',img_gery_2)#,time=2000)
#                 #     cv2.imencode('.jpg', img_gery_2)[1].tofile('img_gery_2')
#                 class_tool.set_path_or_image(None, img_gery_2)
#                 result_list_2, list_confidence_2 = class_tool.print_out()
#                 result_2 = result_list_2[0]
#             if 46388 == pre_time and current_time == 46588:
...
#     return true_of_flag#[0]+true_of_flag[1]+true_of_flag[2]#,true_of_flag[1],true_of_flag[2])#true_of_flag, false_of_flag


def Loss3(platform,model,img,class_tool=None,deblur_tool=None,frame_counter = 0,strat_list = [],index_1 = 0,pre_time=0,current_time=0):
    # if frame_counter < 1396:
    #     return False
    # if frame_counter == 0:
    #     return False
    # print('')
    # print('loss3timebegin',datetime.datetime.now())
    t_2 = 9999999
    img_copy = img.copy()
    # showImg(str(current_time),img)
    # showImg(str(pre_time),model)
    ...
    # imageModel_before = cv2.imread(python_path+'/model_8_before.jpg')
    # img = cv2.imread(python_path+'/model_8.jpg')#
    # img = reShape(imageModel_before,img)
    # cv2.imencode('.jpg', img)[1].tofile('model_tt.jpg')

    img = img[0: img.shape[0],int(img.shape[1]/5*2):img.shape[1]]
    model = model[0: model.shape[0],int(model.shape[1]/5*2):model.shape[1]]
    # img_gery = img
    # model_gery = model
    # # print('model_color.shape',model_color.shape)
    # if len(img_color.shape) == 3:
    #     img_gery = cv2.cvtColor(img_color, cv2.COLOR_RGB2GRAY)
    #     rct, img_gery = cv2.threshold(img_gery, 0, 1, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    # if len(model_color.shape) == 3:
    #     model_gery = cv2.cvtColor(model_color, cv2.COLOR_RGB2GRAY)
    #     rct, model_gery = cv2.threshold(model_gery, 0, 1, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    
    
    t = np.sum(np.sqrt(img-model))
    # print("t=",t)
    flag = False
    threshold_1 = 10000
    threshold_2 = 30000
    if platform == 'ios':
        threshold_1 = 2000
        threshold_2 = 20000
    if t < threshold_1:#/((2248*1080)/(1334*750))):
        # print('t<10000')
        return True
    elif t < threshold_2:#((2248*1080)/(1334*750))):
        time_now = datetime.datetime.now()
        true_of_flag = recog(img_color, model_color, class_tool=class_tool, deblur_tool=deblur_tool,pre_time=pre_time,current_time=current_time)
        # print('true_of_flag',true_of_flag)
        # print('true_of_flag,huafei nei,pre_time',true_of_flag,datetime.datetime.now()-time_now,pre_time)
        return true_of_flag == 9#>= 2
    # print('t=',t)
    # print('loss3timeend',datetime.datetime.now())
    # print('')
    return False


def toRecordDealy(platform,listsOfImg, cnt, thIfDelay, preImage, pre_time,pre_id, dealy_ms, counter_all,
    ...
    # print('loss_2 = ',loss2)
    loss3 = Loss3(platform,preImage,currentImage,platform, class_tool,deblur_tool,couter_frame_wode,pre_time=pre_time,current_time=current_time)#,strat_list=strat_list,index_1=index_)
    # print('loss_3 = ',loss3)
...
def binary_find_the_same_frame(platform,first_reserve,current_frame,cnt,class_tool,deblur_tool,detectrou_tool,flag = 'headortail',listsOfImg_position=[],pre_time= -1,imageModel=None,must_tail=False,process_id=0):
    time_now = datetime.datetime.now()
    t = time_now-time_now
    ...
    #if process_id == 2:# or process_id == 0:
    # if process_id == 2:# or process_id == 0:
    # print('begin_time,end_time,flag,must_tail,process_id',first_reserve[0][0],first_reserve[-1][0],flag,must_tail,process_id)
    
    # if flag=='head':
    #     now_image = first_reserve[-2][1]
    ...
    #     if loss3 == False:
    #         return first_reserve[-1:],first_reserve[-1][0]
    ...
    while begin+1 < end:
        ...
        # print('middle',middle,first_reserve[middle][0])
        # if flag == 'tail':
        ...
        loss3 = Loss3(platform,current_frame,now_image,class_tool,deblur_tool,pre_time =pre_time,current_time=first_reserve[middle][0])#,couter_frame_wode)#,strat_list=strat_list,index_1=index_)
        # print('huafei wai',datetime.datetime.now()-time_now)
        ...
        if loss3 == True:
            ...
            if flag == 'tail' :
                if  must_tail == False and abs(pre_time-first_reserve[middle][0]) < 200:
                    # print('binary_find_the_same_frame',t1,t2)
                    # print('huafei',datetime.datetime.now()-time_now)
                    return first_reserve,-1
                end = middle
            else :
                begin = middle
    ...
    # print('binary_find_the_same_frame',t1,t2)
    # print('huafei',datetime.datetime.now()-time_now)
    # print('shijian',flag,first_reserve[end][0])
    return first_reserve[end:],first_reserve[end][0]

import csv


def update_pre_and_first_reserve(platform,first_reserve, pre_image, currentImage, pre_time, class_tool, deblur_tool,
                                 detectrou_tool, cnt, dealy_ms, listsOfImg_position=[], imageModel=None,
                                 first_200=False, process_id=0):
    loss3 = Loss3(platform,pre_image, currentImage, class_tool=class_tool, deblur_tool=deblur_tool, pre_time=pre_time,
                  current_time=first_reserve[-1][0])

    # True表示是相同的帧，false表示是不同的帧内容
    if False == loss3:
        # 求第一帧的尾，time_now尾的是下一帧的时间
        first_reserve, time_now = binary_find_the_same_frame(platform,first_reserve, pre_image, cnt, class_tool, deblur_tool,
                                                             detectrou_tool, flag='tail',
                                                             listsOfImg_position=listsOfImg_position,
                                                             pre_time=pre_time, imageModel=imageModel,
                                                             must_tail=first_200, process_id=process_id)
        if first_200 == True:
        ...
        if False == first_200:
            if delay >= 200:
                ...
                dealy_ms[0] = dealy_ms[0] + delay
                print('200 pre_time is {},time_now is {},process id is {}'.format(pre_time, time_now, process_id))
            if delay >= 300:
                # print("process id is",process_id)
                dealy_ms[1] = dealy_ms[1] + delay
                print('300 pre_time is {},time_now is {},process id is {}'.format(pre_time, time_now, process_id))
            if delay >= 600:
                # print("process id is",process_id)
                print('600 pre_time is {},time_now is {},process id is {}'.format(pre_time, time_now, process_id))
                dealy_ms[2] = dealy_ms[2] + delay
        first_200 = False
        # currentImage是first_reserve最后一帧的roi， 也就是将要求头的那一阵
        pre_image = currentImage.copy()
        # 更新数组，找到和pre_image相同的帧，将其之前的帧都删除掉
        # print('second')
        # 求最后一帧的头
        # 如果现在首尾相同的话直接返回
        now_image = first_reserve[0][1].copy()
        listsOfImg, listsOfImg_position = pregetTimeresultOrDelay(now_image, imageModel,
                                                                  listsOfImg_position=listsOfImg_position,
                                                                  detectrou_tool=detectrou_tool)
        now_image, thIfDelay = getDelayResultImage(listsOfImg, cnt)
            
        loss3 = Loss3(platform,currentImage, now_image, class_tool, deblur_tool, pre_time=first_reserve[0][0],
                      current_time=first_reserve[-1][0])
        ...
        # 求最后一帧的头，time——now记录的是当前求到的头的位置，first_reserve里面装的是从头开始的、都相同的帧
        first_reserve, time_now = binary_find_the_same_frame(platform,first_reserve, currentImage, cnt, class_tool,
                                                             deblur_tool, detectrou_tool, flag='head',
                                                             listsOfImg_position=listsOfImg_position,
                                                             imageModel=imageModel, process_id=process_id )
...
def half_of_video_deal(begin_frame_counter, end_frame_counter, path,platform, comm, process_id=0):
    import datetime

    imageModel, contours_model_result = getModelJPEGAnd15Positions(python_path+'/model_8.jpg')
    print('len(contours_model_result)', len(contours_model_result))
    ...
    while capture.isOpened():
        if len(first_reserve) == 0 or end_time - first_reserve[0][0] < 200:
            ...
            if counter_of_frame > end_frame_counter:
                break
            # if counter_of_frame%100 == 0:# int((begin_frame_counter+end_frame_counter)/2)
            #     comm.barrier()
            if not frame_exists:
                print('because end')
                break
            ...
            if pre_image is None:
                # comm.barrier()
                # print('end_time',end_time)
                # showImg('now',current_frame)
                listsOfImg, listsOfImg_position = pregetTimeresultOrDelay(current_frame, imageModel, flag_of_path,
                                                                          listsOfImg_position,
                                                                          detectrou_tool=detectrou_tool)
                # t = 0
            ...
            # showImg('first',pre_image)
            # showImg('second',currentImage)
            first_reserve, pre_image, pre_time, dealy_ms, first_200 = update_pre_and_first_reserve(platform,first_reserve,
                pre_image, currentImage, pre_time, class_tool, deblur_tool, detectrou_tool, contours_model_result[0],
                dealy_ms, listsOfImg_position, imageModel, first_200, process_id)
            #comm.barrier()
    ...
    print(datetime.datetime.now())
    print('result_dealy_ms,process_id',process_id,dealy_ms)
    print(end_time)
    # print(path)
    ...
    listsOfImg, listsOfImg_position = pregetTimeresultOrDelay(first_reserve[-1][1], imageModel, flag_of_path,
                                                              listsOfImg_position, detectrou_tool=detectrou_tool)
    currentImage, thIfDelay = getDelayResultImage(listsOfImg, contours_model_result[0])
    first_reserve, pre_image, pre_time, dealy_ms, _ = update_pre_and_first_reserve(platform,first_reserve, pre_image,
                                                                                   currentImage, pre_time, class_tool,
                                                                                   deblur_tool, detectrou_tool,
                                                                                   contours_model_result[0], dealy_ms,
                                                                                   listsOfImg_position, imageModel,
                                                                                   process_id=process_id)
    # 最尾部的，一定是加pre_time first_reserve装的是最后一帧图像以及他的时间。
    first_reserve_list.append((pre_time, first_reserve[-1][0], currentImage))
    return dealy_ms, first_reserve_list


def get_frame_count_of_video(path):
...
python_path = get_env('PYTHONPATH', "/Users/mtest/opencv_bitbucket/timestampocr").split(':')[0]
result_half, first_reserve_list = [], []
...
def main_to_call(path, begin_frame, end_frame, platform,comm, process_id=0):
    print('process_id,begin_frame,end_frame', process_id, begin_frame, end_frame)
    # if process_id != 2:# or process_id != 1:#not(process_id == 5 or process_id == 5):
    #     return  
    return half_of_video_deal(begin_frame, end_frame, path, platform,comm, process_id)
