...
def pregetTimeresultOrDelay(img_color, imageModel, flag_of_path, kind='timeresult',listsOfImg_position=[], detectrou_tool=None):
    listsOfImg = []
    img_result = []
    ...
    return listsOfImg,listsOfImg_position,list_point
...
def getResultRromList(result_list, list_confidence):
    ...
    for i in range(len(result_of_time[0])):
        ...
        duliang = duliang / 10
    result_final = first_num - second#(first_num + 100000 - second_num) % 100000
...
def toRecordDealy(listsOfImg, cnt, thIfDelay, preImage, pre_time,pre_id, dealy_ms, counter_all, frame_end_time,couter_frame_wode,class_tool,deblur_tool,strat_list = [],index_ = 0 ):
    ...
    if currentImage is None:
        ...
        return None, pre_time,pre_id, dealy_ms, counter_all
    ...
    return currentImage.copy(), pre_time, pre_id,dealy_ms, counter_all,1
...
if __name__ == '__main__':
    ...
    path = '/Users/xhj/Downloads/VIDEO_144318.mp4'#31_rec_tag.mp4'
    ...
    print('begin_time_now',begin_time_now)
    while capture.isOpened():
        ...
        # print("timestape", end_time)

        # if end_time < 23766-100:
        #     continue
        # ,'delay')#last return  listsOfCnts
        listsOfImg,listsOfImg_position,list_point = pregetTimeresultOrDelay(current_frame, imageModel, flag_of_path,listsOfImg_position,detectrou_tool=detectrou_tool)
        ...
        # preImage, pre_time,pre_id, dealy_ms, counter_all ,index= toRecordDealy(listsOfImg, contours_model_result[0], thIfDelay,
                                                                # preImage, pre_time,pre_id ,dealy_ms, counter_all,end_time,couter_frame_wode,class_tool,deblur_tool,strat_list = [] ,index_ =index)
        # time.sleep(1)
        couter_frame_wode = couter_frame_wode + 1
        if preImage is None:
            break
