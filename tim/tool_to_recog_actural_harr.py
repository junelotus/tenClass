...
for path in path_switch:
    ...
    img_to_harr_gery_f32 = np.float32(img_to_harr_gery)
    ...
    thread_to_harr = int(shape_harr[0]/10)
    corrdia_list_result =[]
    for position in corrdia_list:
        x = position[0]
        y = position[1]
        if x <= thread_to_harr or x >= shape_harr[0] - thread_to_harr  or ( y <=  thread_to_harr or y >= shape_harr[1] - thread_to_harr):
