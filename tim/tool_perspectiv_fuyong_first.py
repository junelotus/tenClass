...
def correctImageByPerspectiveUseCnt(img, cnt, width, height,sub_corner_list):
    # 获取包围contour的多边形, 应该是个4边形
    (x, y, w, h) = cv2.boundingRect(cnt)
    if len(sub_corner_list) == 4:
        quad_pts = sub_corner_list
    else :
        EPSILON = 5
...
def correctImageByPerspectiveUseCornerOrHough(img_to_correct,img_result,flag_of_path = 0,sub_corner_list=[]):
    ...
    if len(canvas_point) == 4:
        ...
        # to return 
        return result_perspective,'perspective',canvas_point
    # else hough to correct image
        ...
        return img_result,'hough',canvas_point