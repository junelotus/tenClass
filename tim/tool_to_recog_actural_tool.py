...
def getTimeresult(path, deblur_tool, class_tool, imageModel, contours_model_result,name_flag):
    ...
    for i in range(len(listsOfImg)):
        ...
        # img_to_harr = img_color[y1:y1+h1+40, x1-10:x1+w1+40]

        # img_to_harr_gery = cv2.cvtColor(img_to_harr, cv2.COLOR_RGB2GRAY)
        # img_to_harr_gery = np.float32(img_to_harr_gery)

        # dst = cv2.cornerHarris(img_to_harr_gery,2,3,0.04)
    ...
    return listsOfCnt, result_list_fiften_result, list_confidence_fiften_result,name_flag,dst_path
...
name_flag = 1
position, result_list, list_confidence,name_flag,dst_path = getTimeresult('./jpg15/switch_1.jpg', deblur_tool, class_tool, imageModel,contours_model_result,name_flag)
print(result_list)
name_flag = name_flag+1
