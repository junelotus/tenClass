...
def getAvaiableImgUseCorner(img_color, listsOfCnt, w, h, step, imageModel, flag_of_path,sub_corner_list):
    ...
    for w1, h1, x1, y1, L2Loss_result in listsOfCnt:
        ...
        img_result, solution,sub_corner_list = correctImageByPerspectiveUseCornerOrHough(img_to_correct, img_result, flag_of_path,sub_corner_list)
        if solution == 'hough':
            continue
    ...
    return listsOfImg,sub_corner_list


# get Avaiable ROI by solution bounding rectangular
def getAvaiableImgUseCnt(img_color, listsOfCntNormalOrder, imageModel, flag_of_path,sub_corner_list=[]):
    # print('getAvaiableImgUseCnt')
    listsOfImg = []
    shape = imageModel.shape
    for cnt in listsOfCntNormalOrder:
        ...
        sub_corner_list, img_roi = correctImageByPerspectiveUseCnt(img_color, cnt, shape[1], shape[0],sub_corner_list)
...
# add four point list to get simplify  the first correct process
def pregetTimeresultOrDelay(img_color, imageModel, flag_of_path, kind='timeresult',listsOfImg_position=[], detectrou_tool=None,corner_list=[]):
    listsOfImg = []
    img_result = []
    ...
    counter = 0
    # sub_corner_list = []#corner_list[counter]
    the_flag_of_update_corner_list = False

    if len(corner_list) == 0:
        the_flag_of_update_corner_list = True
    ...
    for img_sub,position in zip(img_result,img_result_position):
        ...
        sub_corner_list = []
        if False == the_flag_of_update_corner_list:
            sub_corner_list = corner_list[counter]
            counter = counter+1

        # print('sub_corner_list',sub_corner_list)
        listsOfImg_sub,sub_corner_list =  getAvaiableImgUseCnt(img_sub, listsOfCntNormalOrder, imageModel, flag_of_path,sub_corner_list)  # path,
        if len(listsOfImg_sub) < 1:
            sub_corner_list=[]
            # sub_corner_list = corner_list[counter]
            # counter = counter+1
            listsOfImg_sub,sub_corner_list = getAvaiableImgUseCorner(img_sub, listsOfCnt, 54, 36, 5, imageModel, flag_of_path,sub_corner_list)  # path, 
        if True == the_flag_of_update_corner_list:
            corner_list.append(sub_corner_list)
        if len(listsOfImg_sub) >= 1:
    ...
    return listsOfImg, listsOfImg_position,corner_list
...
if __name__ == '__main__':
    ...
    path = '/Users/xhj/Downloads/hongmi_360_tcoff.mp4'
    ...
    listsOfImg = []
    corner_list= []
    # listsOfImg_position_record = []
    # 找到第一帧改变的
    print(datetime.datetime.now())
    while capture.isOpened():
        # if counter_frame % 12 == 0:
        ...
        listsOfImg, listsOfImg_position,corner_list = pregetTimeresultOrDelay(current_frame, imageModel, flag_of_path,listsOfImg_position,detectrou_tool=detectrou_tool,corner_list=corner_list)
